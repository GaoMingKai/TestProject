...
class Calc041(unittest.TestCase):
    ...
    def checkHistoryData(self,point):
        ...
        try:
            ...
            print(e.__str__())
            self.errors.append("错误信息greenslopes项目%s接口失败,%d秒内没有响应!" % (addr, 20))
                ...
                for point in pointNames:
                    ...
                    if(values):
                        ...
                        if v<=0 or v>values[0]*10:
                            self.errors.append("错误信息greenslopes项目p%s点在%s~%s点值发生突变或者一直没有递增!" % (point, timeStamp[0], timeStamp[1]))
                        # break
            else:
                self.errors.append("错误信息greenslopes项目%s点在%s~%s没有历史数据"% (point, timeStart, timeEnd))
        else:
            self.errors.append("错误信息greenslopes项目%s点在%s~%s没有数据!" % (point, timeStart, timeEnd))
