...
class Calc028(unittest.TestCase):
    ...
    def Test(self):
        ...
        if(self.project):
            self.a.raiseError(self.error, self.testCaseID)
            #BeopTools.send_email(self.emails, self.testCaseID, [s]+self.error, self.testCaseID)
                ...
                if rv and rv.get('success',0):
                    ...
                    for value in values:
                        if(value.get('pointValue',0)=='None') and "test" not in value.get("value").lower():
                            if(n==0):
                                self.error.append('错误信息%s项目中的点有空值:'%param[1])
                            n=n+1
                            self.error.append(value['value'])
                else:
                    self.error.append("错误信息%s接口, 参数: %s 返回预期success为true实际为%s" % (self.url, json.dumps(data), json.dumps(rv.get('success'))))
            except Exception as e:
                print(e.__str__())
                self.error.append('错误信息%s' % (e.__str__()))
